...
This old code was abandoned and entirely re-written ten years later in preparation for Open Sauce 2026.

# components
A lot of the parts used are either EOL or the shopping links have long since changed. None of the parts are particularly specialized though, so here's a list of the major components and some of the reasoning behind the choices:
...
Coils:
Through a lot of empirical testing at this particular voltage, we got the best results using a coil about the length of the projectile. 3 layers of 14 gauge wire worked best for the first two coils, and we noticed a slight improvement dropping it down to 2 layers for the 3rd coil (for higher current). We found that winding the coils consistently and carefully actually got us another foot or two per second on average. 
